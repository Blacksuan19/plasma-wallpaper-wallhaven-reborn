function saveEntry(ctx, imageUrl, thumbnailUrl, localPath) {
    ...
    if (sourceName.startsWith("mkdir")) {
        if (exitCode !== 0) {
            ...
            for (let url in ctx.pendingDownloads) {
                ...
                saveEntry(ctx, url, info.thumbnail, "");
        ...
        for (let url in ctx.pendingDownloads) {
            if (sourceName.includes(url)) {
                ...
                if (exitCode === 0) {
                    ...
                    saveEntry(ctx, url, info.thumbnail, info.localPath);
                } else {
                    if (ctx.log)
